...
public class BadHostnamesGenerator extends ComposedStringGenerator {
	...
	public BadHostnamesGenerator(List<FuzzingHeuristic> owners, long seed, StringSpecification stringSpec) {
	...
	private void initHeuristics() {
		...
		StringRepeater strRepeater_A_1 = new StringRepeater(stringSpec, owners, seed,
				new StringRepeater.Builder().startIndex(1));
		heuristics.add(strRepeater_A_1);

		StringRepeater strRepeater_A_2 = new StringRepeater(stringSpec, owners, seed,
				new StringRepeater.Builder().stepSize(100).size(100).startIndex(100));
		heuristics.add(strRepeater_A_2);

		StringRepeater strRepeater_AdotAdotAdotAdotAdot_ = new StringRepeater(stringSpec, owners, seed,
		...
		StringRepeater strRepeater_dot_ = new StringRepeater(stringSpec, owners, seed,
				new StringRepeater.Builder().string(".").size(10).startIndex(1));
		heuristics.add(strRepeater_dot_);

		StringRepeater strRepeater_dot_20 = new StringRepeater(stringSpec, owners, seed,
				new StringRepeater.Builder().string(StringUtil.repeat(".", 20)).size(20).startIndex(1));
		heuristics.add(strRepeater_dot_20);

		StringRepeater strRepeater_AAAAA_20 = new StringRepeater(stringSpec, owners, seed,
		...
		Combinator aaaaDotAaaaaDotAaaaa = new Combinator(stringSpec, seed, owners, strRepeater_AAAAA_20, dotGenerator,
				strRepeater_AAAAA_20);
		heuristics.add(aaaaDotAaaaaDotAaaaa);

		ConcreteValuesGenerator tld = new ConcreteValuesGenerator(stringSpec, seed, owners, "AC", "AD", "AE", "AERO",
		...
		ConcreteValuesGenerator aaaaDot = new ConcreteValuesGenerator(stringSpec, seed, owners, "AAAA.");
		Combinator aaaaDotTld = new Combinator(stringSpec, seed, owners, aaaaDot, tld);
		heuristics.add(aaaaDotTld);

		heuristics.add(new ConcreteValuesGenerator(stringSpec, seed, owners, "localhost"));
	}
