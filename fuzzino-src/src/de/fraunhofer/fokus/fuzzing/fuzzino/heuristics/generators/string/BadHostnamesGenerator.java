...
public class BadHostnamesGenerator extends ComposedStringGenerator {
	...
	private void initHeuristics() {
		...
		StringRepeater strRepeater_A_1 = new StringRepeater(stringSpec, owner, seed, new StringRepeater.Builder()
		                                                                     .startIndex(1));
		heuristics.add(strRepeater_A_1);
		
		StringRepeater strRepeater_A_2 = new StringRepeater(stringSpec, owner, seed, new StringRepeater.Builder()
                                                                             .stepSize(100)
                                                                             .size(100)
                                                                             .startIndex(100));
		heuristics.add(strRepeater_A_2);
		
		StringRepeater strRepeater_AdotAdotAdotAdotAdot_ = new StringRepeater(stringSpec, owner, seed, new StringRepeater.Builder()
		                                                                                       .string(StringUtil.repeat("A.", 5))
		                                                                                       .size(100)
		                                                                                       .startIndex(1));
		heuristics.add(strRepeater_AdotAdotAdotAdotAdot_);
		
		StringRepeater strRepeater_dot_ = new StringRepeater(stringSpec, owner, seed, new StringRepeater.Builder()
		                                                                      .string(".")
		                                                                      .size(10)
		                                                                      .startIndex(1));
		heuristics.add(strRepeater_dot_);
		
		StringRepeater strRepeater_dot_20 = new StringRepeater(stringSpec, owner, seed, new StringRepeater.Builder()
		                                                                        .string(StringUtil.repeat(".", 20))
		                                                                        .size(20)
		                                                                        .startIndex(1));
		heuristics.add(strRepeater_dot_20);
		
		StringRepeater strRepeater_AAAAA_20 = new StringRepeater(stringSpec, owner, seed, new StringRepeater.Builder()
		                                                                          .string(StringUtil.repeat("A", 5))
		                                                                          .size(20)
		                                                                          .startIndex(1));
		ConcreteValuesGenerator dotGenerator = new ConcreteValuesGenerator(stringSpec, seed, owner, ".");
		Combinator aaaaDotAaaaaDotAaaaa = new Combinator(stringSpec, seed, owner, strRepeater_AAAAA_20, dotGenerator, strRepeater_AAAAA_20);
		heuristics.add(aaaaDotAaaaaDotAaaaa);
		
		ConcreteValuesGenerator tld = new ConcreteValuesGenerator(stringSpec, seed, owner,
				"AC",	"AD",	"AE",	"AERO",	"AF",
				"AG",	"AI",	"AL",	"AM",	"AN",
				"AO",	"AQ",	"AR",	"ARPA",	"AS",
				"AT",	"AU",	"AW",	"AX",	"AZ",
